...
struct tumpukan  //struktur tumpukan
{
 char data[15][100], max[15]; // data[i][j], max[i]
  char data_1[15][100], max1[15]; // data[i][j], max[i]
 int i, j;
} stack;
...
void push()  //push untuk mengisi data
{
 ...
 cout << "Masukan data : ";
 cin >> stack.max;
 strcpy (stack.data[stack.i], stack.max);  //perintah menyalin data char panjang menggunakan fungsi string

  stack.i++;
 cout << "Masukan data kedua : ";
 cin >> stack.max1;
 strcpy (stack.data_1[stack.i], stack.max1);  //perintah menyalin data char panjang menggunakan fungsi string
}
